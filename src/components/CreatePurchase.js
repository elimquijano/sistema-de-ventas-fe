...
export const CreatePurchase = ({ onSuccess, onCancel }) => {
  const { user } = useAuth();
  ...
  const handleSavePurchase = async () => {
    ...
    try {
        await purchasesAPI.create(formData); // Asumo que existe este método
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={7}>
          ...
          <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
            <Button variant="outlined" color="secondary" size="large" onClick={onCancel} disabled={isSaving}>
              Cancelar
            </Button>
